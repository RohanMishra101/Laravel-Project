                                    ...
                                    <div class="col-md-3">
                                        <div class="card mb-4">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <p class="card-text overflow-hidden" style="max-height: 40px">
                                                ...
                                                @foreach ($cartDetails as $details)
                                                    @if ($details->product_id == $product->id)
                                                        <p class="card-text"><small class="text-muted">No. Of
                                                                Orders:{{ $details->no_of_orders }}</small></p>
                                                        @if ($details['order_confirm'] == 0)
                                                            <form action="/cartItemConfirm/{{ $details['id'] }}"
                                                                method="post" style="display: inline">
                                                                @csrf
                                                                <button type="submit"
                                                                    id="showConfirm-{{ $details['order_confirm'] }}"class="btn btn-primary"
                                                        ...
                                                        <div id="hiddenDeleteForm-{{ $details['id'] }}"
                                                            style="display: none">
                                                            <form action="/cartItemDelete/{{ $details['id'] }}"
                                                                method="post">
                                                                @csrf()
